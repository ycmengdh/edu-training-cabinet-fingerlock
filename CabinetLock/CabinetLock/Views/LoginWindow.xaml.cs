...
namespace CabinetLock
{
    ...
    public partial class LoginWindow : BorderlessWindow
    {
        ...
        private void ApplyDefaultCredentials()
        {
            ...
            string path = System.IO.Path.Combine(AppContext.BaseDirectory, "remembered_login.json");
            if (System.IO.File.Exists(path))
            {
                try
                {
                    string json = System.IO.File.ReadAllText(path);
                    var creds = Newtonsoft.Json.JsonConvert.DeserializeObject<dynamic>(json);
                    if (creds != null)
                    {
                        UserIdBox.Text = creds?.UserId ?? "";
                ...
                {
                    // fall through to empty if parse fails
                }
            }

            // 调试阶段已过，不再默认填 admin/admin123
            UserIdBox.Text = "";
        ...
        {
            bool show = PasswordVisibilityToggle.IsChecked ?? false;
            ...
            if (show)
            {
                ...
                PasswordVisibilityToggle.Content = "\uE891";
                PasswordVisibilityToggle.ToolTip = "隐藏密码";
            }
            else
            {
                ...
                PasswordVisibilityToggle.Content = "\uE890";
                PasswordVisibilityToggle.ToolTip = "显示密码";
            }
        }

        private void SaveRememberedCredentials()
        {
            if (!(RememberPasswordCheckBox?.IsChecked ?? false)) return;

            var creds = new { UserId = UserIdBox.Text?.Trim() ?? "", Password = GetEnteredPassword() };
            try
            {
