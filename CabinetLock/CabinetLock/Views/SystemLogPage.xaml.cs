...
namespace CabinetLock
{
    ...
    public partial class SystemLogPage : Page
    {
        ...
        private int _opPageSize = 50;
        private int _commPageSize = 50;
        private int _unlockPageSize = 50;
        ...
        private async void OpPager_PageRequested(object sender, Controls.PaginationRequestedEventArgs e)
        {
            _opPage = e.PageIndex;
            _opPageSize = e.PageSize;
            await LoadOpLogsAsync();
        }
        ...
        private async Task LoadOpLogsAsync()
        {
            ...
            try
            {
                ...
                int totalPages = Math.Max(1, (int)Math.Ceiling(total / (double)_opPageSize));
                if (_opPage >= totalPages) _opPage = totalPages - 1;
                if (_opPage < 0) _opPage = 0;

                var list = await Task.Run(() =>
                    App.OperationLogService.Query(keyword, start, end, _opPageSize, _opPage * _opPageSize));

                OpDataGrid.ItemsSource = list;
                OpStatusText.Text = $"共 {total} 条，本页 {list.Count} 条";
                OpPager.Configure(total, _opPage, _opPageSize);
        ...
        private async void CommPager_PageRequested(object sender, Controls.PaginationRequestedEventArgs e)
        {
            _commPage = e.PageIndex;
            _commPageSize = e.PageSize;
            await LoadCommLogsAsync();
        }
        ...
        private async Task LoadCommLogsAsync(bool quiet = false)
        {
            ...
            try
            {
                ...
                int total = all.Count;
                int totalPages = Math.Max(1, (int)Math.Ceiling(total / (double)_commPageSize));
                if (_commPage >= totalPages) _commPage = totalPages - 1;
                if (_commPage < 0) _commPage = 0;
                var page = all.Skip(_commPage * _commPageSize).Take(_commPageSize).ToList();

                CommDataGrid.ItemsSource = page;
                CommStatusText.Text = $"共 {total} 条（内存缓存），本页 {page.Count} 条";
                CommPager.Configure(total, _commPage, _commPageSize);
        ...
        private async void UnlockPager_PageRequested(object sender, Controls.PaginationRequestedEventArgs e)
        {
            _unlockPage = e.PageIndex;
            _unlockPageSize = e.PageSize;
            await LoadUnlockLogsAsync();
        }
        ...
        private async Task LoadUnlockLogsAsync()
        {
            ...
            try
            {
                ...
                int totalPages = Math.Max(1, (int)Math.Ceiling(total / (double)_unlockPageSize));
                ...
                var list = await Task.Run(() =>
                    App.LogService.QueryVisibleLogs(deviceId, userId, start, end, result,
                        _unlockPageSize, _unlockPage * _unlockPageSize, keyword));

                UnlockDataGrid.ItemsSource = list;
                UnlockStatusText.Text = $"共 {total} 条，本页 {list.Count} 条";
                UnlockPager.Configure(total, _unlockPage, _unlockPageSize);
        ...
        private async void UnlockExportButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                var rows = all.Select(x => (IReadOnlyList<object?>)new object?[]
                {
                    x.CreateTime, x.DeviceId, x.DisplayUserId, x.LockDisplay, x.Action, x.Result, x.Reason
                });
        ...
        private void SetBusy(bool busy, string? status = null)
        {
            ...
            OpPager.IsEnabled = !busy;
            CommPager.IsEnabled = !busy;
            UnlockPager.IsEnabled = !busy;
