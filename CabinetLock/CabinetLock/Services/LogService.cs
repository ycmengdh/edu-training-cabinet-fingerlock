namespace CabinetLock
{
    ...
    public class LogService
    {
        ...
        {
            return ApplyUserCodes(LogDatabase.QueryUnlock(
                deviceId, ResolveInternalUserId(userId), startTime, endTime, result, limit, offset)).ToList();
        }

        public int CountLogs(string? deviceId = null, string? userId = null,
            DateTime? startTime = null, DateTime? endTime = null, string? result = null)
        {
            return LogDatabase.CountUnlock(
                deviceId, ResolveInternalUserId(userId), startTime, endTime, result);
        }
            ...
            DateTime? startTime, DateTime? endTime, string? result)
        {
            ...
            return LogDatabase.QueryUnlock(deviceId, ResolveInternalUserId(userId), startTime, endTime, result,
                limit: int.MaxValue, offset: 0);
        }

        private static string? ResolveInternalUserId(string? userId)
        {
            if (string.IsNullOrWhiteSpace(userId)) return null;
            string value = userId.Trim();
            try { return App.UserService.GetUserByCode(value)?.UserId ?? value; }
            catch { return value; }
        }
