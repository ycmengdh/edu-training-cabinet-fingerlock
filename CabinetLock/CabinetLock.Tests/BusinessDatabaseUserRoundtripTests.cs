    ...
    public void Users_RoundtripGenderAndAssignedDeviceIds()
    {
        ...
        try
        {
            ...
            [{
              ...
              "cabinet_assignments":[
                {"device_id":"CAB_01","fingerprint_ids":[12],"lock_ids":[1],"update_time":"2026-07-27T08:10:00+08:00"},
                {"device_id":"CAB_02","fingerprint_ids":[18],"lock_ids":[2,3],"update_time":"2026-07-27T08:11:00+08:00"}
              ],
              "fingerprint_id":12,
              "password_salt":"",
              "password_hash":"",
            ...
            Assert.Equal(new[] { 12, 18 }, stored["cabinet_assignments"]!
                .Select(item => item!["fingerprint_ids"]!.Values<int>().Single()));

            User user = stored.ToObject<User>()!;
            Assert.Equal("female", user.Gender);
            Assert.Equal(new[] { "CAB_01", "CAB_02" }, user.AssignedDeviceIds);
            Assert.Equal(new[] { 18 }, user.CabinetAssignments![1].FingerprintIds);
            Assert.Equal(new[] { 2, 3 }, user.CabinetAssignments![1].LockIds);
        }
        finally
        {
