...
bool LockControl::openLock(int lockId) {
    ...
    lockOpenTime[lockId] = millis();
    updateShiftRegister();
    Debug::printf("[LOCK] Open lock %d (relay LOW, LED on)\n", lockId);
    return true;
}
...
void LockControl::closeLock(int lockId) {
    ...
    if (lockActive[lockId]) {
        Debug::printf("[LOCK] Close lock %d (relay HIGH, LED off)\n", lockId);
    }
