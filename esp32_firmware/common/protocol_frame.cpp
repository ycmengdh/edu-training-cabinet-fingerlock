...
int ProtocolFrame::getEncodedCapacity(const String &json) {
    ...
    if (jsonLen > FRAGMENT_REASSEMBLY_BUF) return -1;

    if (jsonLen <= FRAME_MAX_PAYLOAD) {
        return FRAME_HEADER_SIZE + FRAME_MAX_PAYLOAD + FRAME_CRC_SIZE;
    }

    size_t chunkSize = FRAME_MAX_PAYLOAD - FRAGMENT_HEADER_SIZE;
    size_t totalFrags = (jsonLen + chunkSize - 1) / chunkSize;
...
int ProtocolFrame::encode(const String &json, uint8_t *outBuf, int outBufSize, uint8_t msgId) {
    size_t jsonLen = json.length();
    if (jsonLen > FRAGMENT_REASSEMBLY_BUF) {
    ...
    int totalFrags = (int)((jsonLen + chunkSize - 1) / chunkSize);
...
String ProtocolFrame::handleFragment(const uint8_t *data, uint16_t len) {
    ...
    // 检查是否是新消息
    if (!fragBuf.active || fragBuf.msgId != msgId || fragBuf.total != total) {
        // 新分片消息
