    ...
    public partial class DevicePage : Page
    {
        ...
        private void ApplyFingerprintFilter()
        {
            ...
            var visible = _fingerprints.Where(item =>
                ...
                (item.UserId?.Contains(keyword, StringComparison.OrdinalIgnoreCase) ?? false) ||
            ...
            VisibleFingerprintCountText.Text = _reportedFingerprintCount.HasValue
                ? $"{visible.Count} 人 / {_reportedFingerprintCount.Value} 枚模板"
                : $"{visible.Count} 人";
        ...
        private async void DeleteSelectedFpButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBox.Show(
                    $"确认删除「{selected.UserName}（{selected.UserId}）」在当前柜子的用户指纹和权限数据？\n\n" +
                    "用户主档及其在其他柜子的绑定不会受影响。",
            ...
            try
            {
                ...
                bool bindingSaved = await Task.Run(() =>
                    App.CabinetBindingService.Remove(_selectedDevice.DeviceId, selected.UserId!));
                if (!bindingSaved)
                {
                    MessageBox.Show(
