...
namespace FingerprintLockManager
{
    ...
    public class FingerprintTemplateService
    {
        ...
        /// <summary>
        /// 获取所有模板：本机业务库 + 用户表中有 fingerprint_id 但库中尚无元数据的占位项。
        /// </summary>
            ...
            string deviceId, int statusTimeoutMs = 2500)
        {
            ...
            foreach (var u in users)
            {
                ...
                int? activeFingerprintId = App.CabinetBindingService
                    .GetActiveFingerprintId(u, deviceId);
                if (!activeFingerprintId.HasValue) continue;
                ...
                {
                    FingerprintId = activeFingerprintId.Value,
                    UserId = u.UserId,
                    UserName = u.Name,
                    Role = u.Role,
                    IsEnabled = u.Enabled,
