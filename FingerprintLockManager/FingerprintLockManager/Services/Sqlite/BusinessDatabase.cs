...
CREATE INDEX IF NOT EXISTS ix_fp_user ON fingerprint_templates(user_id);
...
";
            ...
            EnsureColumn(conn, "fingerprint_templates", "finger_name", "TEXT NOT NULL DEFAULT ''");
            EnsureColumn(conn, "fingerprint_templates", "quality", "INTEGER NOT NULL DEFAULT 0");
            EnsureColumn(conn, "fingerprint_templates", "enabled", "INTEGER NOT NULL DEFAULT 1");
...
ON CONFLICT(table_name) DO UPDATE SET version=$v, updated_at=$u;";
        ...
        public static void MigrateFingerprintsFromLocalCacheIfEmpty()
        {
            lock (Sync)
            {
                ...
                {
                    countCmd.CommandText = "SELECT COUNT(1) FROM fingerprint_templates";
        ...
        public static bool DeleteFpTemplateByFingerprintId(int fingerprintId)
        {
            ...
            lock (Sync)
            {
                ...
                using var conn = Open();
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "DELETE FROM fingerprint_templates WHERE fingerprint_id=$id";
...
FROM fingerprint_templates";
        ...
        private static List<FingerprintTemplate> ReadAllFpMetasUnlocked(SqliteConnection conn)
        {
            ...
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"SELECT fingerprint_id,user_id,user_name,finger_index,finger_name,
quality,enabled,enroll_time,template_size,source_device,backup_status,note FROM fingerprint_templates
ORDER BY enroll_time DESC, fingerprint_id DESC";
            ...
            SqliteConnection conn, int fingerprintId, SqliteTransaction? transaction = null)
        {
            ...
            cmd.CommandText = "SELECT template_blob FROM fingerprint_templates WHERE fingerprint_id=$id";
            cmd.Parameters.AddWithValue("$id", fingerprintId);
            var o = cmd.ExecuteScalar();
...
ON CONFLICT(fingerprint_id) DO UPDATE SET
  ...
  note=excluded.note,
  template_blob=COALESCE(excluded.template_blob, fingerprint_templates.template_blob);";
...
ON CONFLICT(table_name) DO UPDATE SET version=version+1,updated_at=$time;";
        ...
        private static JArray ReadUsers(SqliteConnection conn)
        {
            var arr = new JArray();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"SELECT user_id,name,gender,role,class_id,assigned_device_ids_json,
cabinet_assignments_json,fingerprint_id,password_salt,password_hash,enabled,create_time,update_time FROM users";
            ...
            while (r.Read())
            {
                var user = new JObject
                {
                    ...
                    ["class_id"] = r.IsDBNull(4) ? null : r.GetString(4),
                    ["assigned_device_ids"] = null,
                    ["cabinet_assignments"] = null,
                    ["fingerprint_id"] = r.IsDBNull(7) ? null : r.GetInt32(7),
                    ["password_salt"] = r.IsDBNull(8) ? "" : r.GetString(8),
                    ["password_hash"] = r.IsDBNull(9) ? "" : r.GetString(9),
                    ["enabled"] = !r.IsDBNull(10) && r.GetInt64(10) != 0,
                    ["create_time"] = r.IsDBNull(11) ? null : r.GetString(11),
                    ["update_time"] = r.IsDBNull(12) ? null : r.GetString(12)
                };
                if (!r.IsDBNull(5))
                {
                    try { user["assigned_device_ids"] = JArray.Parse(r.GetString(5)); }
                    catch { user["assigned_device_ids"] = new JArray(); }
                }
                if (!r.IsDBNull(6))
                {
                    try { user["cabinet_assignments"] = JArray.Parse(r.GetString(6)); }
...
last_seen,offline_time,mesh_mac,is_root,firmware_version,status_json FROM devices";
        ...
        private static void WriteUsers(SqliteConnection conn, SqliteTransaction tx, JArray array)
        {
            ...
            foreach (var token in array.OfType<JObject>())
            {
                using var cmd = conn.CreateCommand();
                cmd.Transaction = tx;
                cmd.CommandText = @"INSERT INTO users(user_id,name,gender,role,class_id,assigned_device_ids_json,
cabinet_assignments_json,fingerprint_id,password_salt,password_hash,enabled,create_time,update_time)
VALUES($id,$n,$g,$r,$c,$d,$bindings,$f,$s,$h,$e,$ct,$ut)";
